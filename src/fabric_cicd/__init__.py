...
def change_log_level(level: str = "DEBUG") -> None:
    ...
    Warning:
        When debug is enabled, the log file generated by fabric-cicd (e.g., ``fabric_cicd.error.log``) may contain
        sensitive information such as API endpoints, workspace identifiers, and request/response payloads.
        You are responsible for managing and deleting these files in accordance with your
        organization's security policies. Do not commit log files to source control or share
        them in unsecured channels.
