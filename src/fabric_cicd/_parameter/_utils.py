...
def build_dynamic_variable_dependency_graph(
    ...
    For each reference to an item that is NOT already deployed but IS in the repository (new item
    in the current batch), creates a dependency edge.

    Args:
        environment_parameter: The parsed parameter file dictionary.
        environment: The target environment (e.g., "DEV", "PPE").
        deployed_items: Dictionary of currently deployed items in the workspace.
        repository_items: Dictionary of items found in the repository.
    ...
    """
    items_pattern = re.compile(r"^\$items\.", re.IGNORECASE)
    edges: list[tuple[str, str]] = []

    for param_name in ("find_replace", "key_value_replace"):
        param_values = environment_parameter.get(param_name)
        if not isinstance(param_values, list):
            continue

        for param_dict in param_values:
            # Extract replace_value entries for the target environment
            replace_value = param_dict.get("replace_value")
            if not isinstance(replace_value, dict):
                continue

            # Process _ALL_ environment key
            processed = process_environment_key(environment, dict(replace_value))
            env_value = processed.get(environment)
            if not isinstance(env_value, str) or not items_pattern.search(env_value):
                continue

            # Parse the referenced item from the $items variable
            ...
            # If the referenced item is already deployed, no dependency edge needed
            if ref_type in deployed_items and ref_name in deployed_items[ref_type]:
                continue

            # If the referenced item is in the repository (new item in batch), create edge
            if ref_type in repository_items and ref_name in repository_items[ref_type]:
                # Determine which items this parameter applies to via filters
                referencing_keys = _get_referencing_item_keys(param_dict, repository_items)
                for referencing_key in referencing_keys:
                    if referencing_key != ref_key:
...
def _parse_items_variable_reference(variable: str) -> tuple[str, str] | None:
    ...
    # New format: $items.type.name.$attribute
    if ".$" in var_string:
        name_part, _attr = var_string.rsplit(".$", 1)
    # Legacy format: $items.type.name.attribute
    elif "." in var_string:
    ...
    # Split name_part into type and name at the first dot
...
def _get_referencing_item_keys(param_dict: dict, repository_items: dict) -> list[str]:
    """
...
def compute_publish_tiers(
    items_with_context: list[tuple[str, object, object]],
    dependency_edges: list[tuple[str, str]],
) -> list[list[tuple[str, object, object]]]:
    """
    Computes publish tiers from a dependency graph using topological sort.

    Items with no dependencies or whose dependencies are already deployed go into Tier 0.
    Items depending on Tier 0 items go into Tier 1, and so on.
    If no dependency edges exist, all items are placed in a single tier (Tier 0).
    ...
    Returns:
        A list of tiers, where each tier is a list of (item_name, Item, ItemPublisher) tuples.

    Raises:
        InputError: If a circular dependency is detected.
    """
    # Fast path: no edges means single tier
    if not dependency_edges:
        return [items_with_context]

    # Build item key → context mapping
    ...
    # Build adjacency and in-degree from edges (only for items in the batch)
    batch_keys = set(item_key_to_context.keys())
    in_degree: dict[str, int] = {k: 0 for k in batch_keys}
    dependents: dict[str, list[str]] = {k: [] for k in batch_keys}

    for referencing, referenced in dependency_edges:
        if referencing in batch_keys and referenced in batch_keys:
    ...
    tiers: list[list[tuple[str, object, object]]] = []
    current_tier_keys = [k for k, deg in in_degree.items() if deg == 0]

    processed = set()
    while current_tier_keys:
        tier = []
        next_tier_keys = []
        for key in current_tier_keys:
            if key in item_key_to_context:
                tier.append(item_key_to_context[key])
            processed.add(key)
            for dependent in dependents.get(key, []):
                in_degree[dependent] -= 1
                if in_degree[dependent] == 0:
                    next_tier_keys.append(dependent)

        if tier:
            tiers.append(tier)
        current_tier_keys = next_tier_keys

    # Detect circular dependencies
    if len(processed) < len(batch_keys):
        cycle_keys = sorted(batch_keys - processed)
